import React from "react";

const footerSections = [
  {
    title: "Estilos Populares",
    links: [
      { label: "Letras Cursivas", href: "/letras-bonitas/letras-cursivas" },
      { label: "Letras Aesthetic", href: "/letras-bonitas/letras-aesthetic" },
      { label: "Letras Japonesas", href: "/letras-bonitas/letras-japonesas" },
      { label: "Letras Graffiti", href: "/letras-bonitas/letras-graffiti" },
      { label: "Alfabeto Cursivo", href: "/alfabeto-completo/alfabeto-cursivo" },
      { label: "Alfabeto Graffiti", href: "/alfabeto-completo/alfabeto-graffiti" },
    ],
  },
  {
    title: "Para Juegos",
    links: [
      { label: "Nick Free Fire", href: "/nicks-para-juegos/nick-free-fire" },
      { label: "Nombres Roblox", href: "/nicks-para-juegos/nombres-para-roblox" },
      { label: "FF Femenino", href: "/nicks-para-juegos/nick-free-fire-femenino" },
      { label: "FF Masculino", href: "/nicks-para-juegos/nick-free-fire-masculino" },
      { label: "Cambiar Nick FF", href: "/nicks-para-juegos/cambiar-nick-free-fire" },
      { label: "Bio Colorida FF", href: "/nicks-para-juegos/bio-colorida-free-fire" },
    ],
  },
  {
    title: "Para Redes Sociales",
    links: [
      { label: "Bio Instagram", href: "/fuentes-para-instagram/bio-para-instagram" },
      { label: "Fuentes Facebook", href: "/fuentes-para-instagram/fuentes-para-facebook" },
      { label: "Fuentes WhatsApp", href: "/fuentes-para-instagram/fuentes-para-whatsapp" },
      { label: "Fuentes Discord", href: "/fuentes-para-instagram/fuentes-para-discord" },
    ],
  },
  {
    title: "Herramientas",
    links: [
      { label: "Invertir Texto", href: "/letras-personalizadas/invertir-texto" },
      { label: "Texto en Negrita", href: "/letras-personalizadas/texto-en-negrita" },
      { label: "Nombres Personalizados", href: "/letras-personalizadas/nombres-personalizados" },
      { label: "Emojis para Copiar", href: "/simbolos-aesthetic/emojis-para-copiar" },
      { label: "Símbolos Coquette", href: "/simbolos-aesthetic/simbolos-coquette" },
    ],
  },
];

const socialLinks = [
  {
    name: "Facebook",
    href: "#",
    icon: (
      <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
        <path fillRule="evenodd" d="M22 12c0-5.523-4.477-10-10-10S2 6.477 2 12c0 4.991 3.657 9.128 8.438 9.878v-6.987h-2.54V12h2.54V9.797c0-2.506 1.492-3.89 3.777-3.89 1.094 0 2.238.195 2.238.195v2.46h-1.26c-1.243 0-1.63.771-1.63 1.562V12h2.773l-.443 2.891h-2.33v6.988C18.343 21.128 22 16.991 22 12z" clipRule="evenodd" />
      </svg>
    ),
  },
  {
    name: "Twitter / X",
    href: "#",
    icon: (
      <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
        <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
      </svg>
    ),
  },
  {
    name: "Instagram",
    href: "#",
    icon: (
      <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
        <path fillRule="evenodd" d="M12.315 2c2.43 0 2.784.013 3.808.06 1.064.049 1.791.218 2.427.465a4.902 4.902 0 011.772 1.153 4.902 4.902 0 011.153 1.772c.247.636.416 1.363.465 2.427.048 1.067.06 1.407.06 4.123v.08c0 2.643-.012 2.987-.06 4.043-.049 1.064-.218 1.791-.465 2.427a4.902 4.902 0 01-1.153 1.772 4.902 4.902 0 01-1.772 1.153c-.636.247-1.363.416-2.427.465-1.067.048-1.407.06-4.123.06h-.08c-2.643 0-2.987-.012-4.043-.06-1.064-.049-1.791-.218-2.427-.465a4.902 4.902 0 01-1.772-1.153 4.902 4.902 0 01-1.153-1.772c-.247-.636-.416-1.363-.465-2.427-.047-1.024-.06-1.379-.06-3.808v-.63c0-2.43.013-2.784.06-3.808.049-1.064.218-1.791.465-2.427a4.902 4.902 0 011.153-1.772A4.902 4.902 0 015.45 2.525c.636-.247 1.363-.416 2.427-.465C8.901 2.013 9.256 2 11.685 2h.63zm-.081 1.802h-.468c-2.456 0-2.784.011-3.807.058-.975.045-1.504.207-1.857.344-.467.182-.8.398-1.15.748-.35.35-.566.683-.748 1.15-.137.353-.3.882-.344 1.857-.047 1.023-.058 1.351-.058 3.807v.468c0 2.456.011 2.784.058 3.807.045.975.207 1.504.344 1.857.182.466.399.8.748 1.15.35.35.683.566 1.15.748.353.137.882.3 1.857.344 1.054.048 1.37.058 4.041.058h.08c2.597 0 2.917-.01 3.96-.058.976-.045 1.505-.207 1.858-.344.466-.182.8-.398 1.15-.748.35-.35.566-.683.748-1.15.137-.353.3-.882.344-1.857.048-1.055.058-1.37.058-4.041v-.08c0-2.597-.01-2.917-.058-3.96-.045-.976-.207-1.505-.344-1.858a3.097 3.097 0 00-.748-1.15 3.098 3.098 0 00-1.15-.748c-.353-.137-.882-.3-1.857-.344-1.023-.047-1.351-.058-3.807-.058zM12 6.865a5.135 5.135 0 110 10.27 5.135 5.135 0 010-10.27zm0 1.802a3.333 3.333 0 100 6.666 3.333 3.333 0 000-6.666zm5.338-3.205a1.2 1.2 0 110 2.4 1.2 1.2 0 010-2.4z" clipRule="evenodd" />
      </svg>
    ),
  },
  {
    name: "Pinterest",
    href: "#",
    icon: (
      <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
        <path fillRule="evenodd" d="M12 0C5.373 0 0 5.372 0 12c0 5.084 3.163 9.426 7.627 11.174-.105-.949-.2-2.405.042-3.441.218-.937 1.407-5.965 1.407-5.965s-.359-.719-.359-1.782c0-1.668.967-2.914 2.171-2.914 1.023 0 1.518.769 1.518 1.69 0 1.029-.655 2.568-.994 3.995-.283 1.194.599 2.169 1.777 2.169 2.133 0 3.772-2.249 3.772-5.495 0-2.873-2.064-4.882-5.012-4.882-3.414 0-5.418 2.561-5.418 5.207 0 1.031.397 2.138.893 2.738.098.119.112.224.083.345-.09.375-.293 1.199-.334 1.363-.053.225-.172.271-.401.165-1.495-.69-2.433-2.878-2.433-4.646 0-3.776 2.748-7.252 7.92-7.252 4.158 0 7.392 2.967 7.392 6.923 0 4.135-2.607 7.462-6.233 7.462-1.214 0-2.354-.629-2.758-1.379l-.749 2.848c-.269 1.045-1.004 2.352-1.498 3.146C9.57 23.812 10.763 24 12 24c6.627 0 12-5.373 12-12 0-6.628-5.373-12-12-12z" clipRule="evenodd" />
      </svg>
    ),
  },
  {
    name: "YouTube",
    href: "#",
    icon: (
      <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
        <path fillRule="evenodd" d="M23.498 6.186a3.016 3.016 0 00-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 00.502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 002.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 002.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z" clipRule="evenodd" />
      </svg>
    ),
  },
];

export default function Footer() {
  return (
    <footer className="w-full bg-neutral-900 dark:bg-neutral-950 text-neutral-300 border-t border-neutral-800 transition-colors mt-auto">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-12 lg:py-16 flex flex-col gap-12">
        {/* Main Grid: Left Brand Column + 4-Column Links Grid */}
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-10 lg:gap-12">
          {/* Left Column: Brand & Tagline */}
          <div className="lg:col-span-1 flex flex-col gap-4">
            <a
              href="/"
              className="text-2xl font-extrabold bg-gradient-to-r from-purple-400 via-pink-400 to-indigo-400 bg-clip-text text-transparent hover:opacity-90 transition-opacity w-fit"
            >
              Letras Bonitas
            </a>
            <p className="text-sm text-neutral-400 leading-relaxed">
              El mejor generador de letras bonitas, tipografías cursivas, símbolos aesthetic y nicks para juegos gratis. Copia y pega tu texto estilizado al instante.
            </p>
          </div>

          {/* 4-Column Link Grid */}
          <div className="lg:col-span-4 grid grid-cols-2 md:grid-cols-4 gap-8">
            {footerSections.map((section) => (
              <div key={section.title} className="flex flex-col gap-3">
                <h3 className="text-xs font-bold uppercase tracking-wider text-white">
                  {section.title}
                </h3>
                <ul className="flex flex-col gap-2 text-sm">
                  {section.links.map((link) => (
                    <li key={link.label}>
                      <a
                        href={link.href}
                        className="text-neutral-400 hover:text-purple-400 transition-colors"
                      >
                        {link.label}
                      </a>
                    </li>
                  ))}
                </ul>
              </div>
            ))}
          </div>
        </div>

        {/* Divider */}
        <div className="w-full h-px bg-neutral-800" />

        {/* Bottom Row: Social Icons & Copyright */}
        <div className="flex flex-col sm:flex-row items-center justify-between gap-6">
          {/* Copyright */}
          <p className="text-sm text-neutral-400 text-center sm:text-left">
            © 2026 Letras Bonitas. Todos los derechos reservados.
          </p>

          {/* Social Icons Row */}
          <div className="flex items-center gap-4 text-neutral-400">
            {socialLinks.map((social) => (
              <a
                key={social.name}
                href={social.href}
                aria-label={social.name}
                className="hover:text-purple-400 transition-colors p-1.5 rounded-lg hover:bg-neutral-800/50"
              >
                {social.icon}
              </a>
            ))}
          </div>
        </div>
      </div>
    </footer>
  );
}
