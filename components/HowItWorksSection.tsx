    ...
    <section className="w-full flex flex-col gap-6 p-6 sm:p-10 rounded-3xl border border-[var(--border-color)] bg-[var(--card-bg)]/90 backdrop-blur-xl shadow-2xl shadow-purple-950/20">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5 pt-2">
        {STEPS.map((step) => (
          <div
            key={step.number}
            className="p-6 rounded-2xl border border-[var(--border-color)] bg-[var(--input-bg)] flex flex-col gap-3 hover:border-teal-400/60 transition-colors"
          >
            <div className="flex items-center justify-between">
              ...
              <span className="w-7 h-7 rounded-full bg-teal-500/10 text-teal-600 dark:text-teal-400 font-black text-xs flex items-center justify-center border border-teal-500/30">
                {step.number}
              </span>
            </div>
            <h3 className="font-extrabold text-base text-[var(--foreground)] pt-1">
              {step.title}
            </h3>
