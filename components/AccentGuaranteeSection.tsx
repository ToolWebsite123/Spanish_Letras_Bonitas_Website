  ...
  return (
    <section className="w-full flex flex-col gap-6 p-6 sm:p-10 rounded-3xl border border-[var(--border-color)] bg-[var(--card-bg)]/90 backdrop-blur-xl shadow-2xl shadow-purple-950/20">
      <div className="flex flex-col gap-2">
        <div className="inline-flex items-center gap-2 self-start px-3.5 py-1 rounded-full bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 font-bold text-xs border border-emerald-500/30">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 pt-2">
        {ACCENT_SAMPLES.map((sample, idx) => (
          <div
            key={idx}
            className="p-5 rounded-2xl border border-[var(--border-color)] bg-[var(--input-bg)] flex flex-col gap-2 hover:border-teal-400/60 transition-colors"
          >
            <span className="text-xs font-black text-teal-600 dark:text-teal-400">
              {sample.accented}
            </span>
            <span className="text-lg font-normal text-[var(--foreground)] font-mono pt-1">
              {sample.exampleWord}
            </span>
            <span className="text-[10px] font-bold text-emerald-700 dark:text-emerald-400 pt-1">
