    ...
    <div className="min-h-screen bg-[#0f0c1b] text-slate-100 transition-colors flex flex-col">
      ...
      <main className="flex-1 py-8 sm:py-12 px-4 sm:px-8 w-full max-w-5xl mx-auto flex flex-col gap-10 sm:gap-14">
        <nav className="flex items-center gap-2 text-xs sm:text-sm text-slate-400 font-medium">
          ...
          <span>/</span>
          <span className="text-teal-400 font-bold">FF Femenino</span>
        </nav>
        ...
        <header className="flex flex-col gap-4">
          ...
          <p className="text-base sm:text-lg text-slate-300 leading-relaxed max-w-4xl font-medium">
            Genera apodos hermosos, estéticos y respetados para liderar tu escuadra femenina en Free Fire. Combina <Link href="/letras-bonitas/letras-cursivas" className="text-teal-400 underline font-bold hover:text-teal-300">fuentes cursivas delicadas</Link> con <Link href="/simbolos-aesthetic/simbolos-coquette" className="text-teal-400 underline font-bold hover:text-teal-300">símbolos coquette de moños y flores</Link>, <Link href="/nicks-para-juegos/bio-colorida-free-fire" className="text-teal-400 underline font-bold hover:text-teal-300">firmas de colores</Link> y <Link href="/nicks-para-juegos/nick-free-fire" className="text-teal-400 underline font-bold hover:text-teal-300">nicks de Free Fire pro</Link>.
          </p>
        </header>
        ...
        <section className="flex flex-col gap-6 p-6 sm:p-10 rounded-3xl border border-purple-900/40 bg-[#1b1530]/90 backdrop-blur-xl shadow-2xl shadow-purple-950/50">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5 text-xs sm:text-sm">
            <div className="p-6 rounded-2xl bg-[#231c3d] border border-purple-900/40 flex flex-col gap-2.5">
              ...
              <h3 className="font-extrabold text-base text-teal-400">1. Estilo Reina (Queen)</h3>
              <p className="text-slate-300 leading-relaxed">
                Coronas y alas delicadas (ej: <code className="text-teal-400 font-bold">꧁♡𝓟𝓻𝓲𝓷𝓬𝓮𝓼𝓼♡꧂</code>).
              </p>
            </div>

            <div className="p-6 rounded-2xl bg-[#231c3d] border border-purple-900/40 flex flex-col gap-2.5">
              <span className="text-2xl">🌸</span>
              <h3 className="font-extrabold text-base text-teal-400">2. Estilo Soft & Aesthetic</h3>
              <p className="text-slate-300 leading-relaxed">
                Flores y lunas suaves (ej: <code className="text-teal-400 font-bold">🌸 𝒬𝓊𝑒𝑒𝓃 🌸</code>).
              </p>
            </div>

            <div className="p-6 rounded-2xl bg-[#231c3d] border border-purple-900/40 flex flex-col gap-2.5">
              <span className="text-2xl">⚡</span>
              <h3 className="font-extrabold text-base text-teal-400">3. Estilo Rusher Femenina</h3>
              <p className="text-slate-300 leading-relaxed">
                Letras góticas con destellos (ej: <code className="text-teal-400 font-bold">⚡ ᴬⁿᵍᵉˡ 𝔽𝔉 ⚡</code>).
              </p>
            </div>
          </div>
        </section>
