    ...
    <div className="min-h-screen bg-[var(--background)] text-[var(--foreground)] transition-colors flex flex-col">
      ...
      <main className="flex-1 py-8 sm:py-12 px-4 sm:px-8 w-full max-w-5xl mx-auto flex flex-col gap-10 sm:gap-14">
        <nav className="flex items-center gap-2 text-xs sm:text-sm text-[var(--foreground)] opacity-75 font-medium">
          ...
          <span>/</span>
          <span className="text-teal-600 dark:text-teal-400 font-bold">Bio Colorida FF</span>
        </nav>
        ...
        <header className="flex flex-col gap-4">
          ...
          <p className="text-base sm:text-lg text-[var(--foreground)] opacity-80 leading-relaxed max-w-4xl font-medium">
            Personaliza tu firma de perfil en Free Fire con colores fucsia, verde neón, dorado y azul rey. Copia los códigos Hexadecimales e imita las banderas de México, España, Argentina y Colombia. Combínalos con <Link href="/nicks-para-juegos/nick-free-fire" className="text-teal-600 dark:text-teal-400 underline font-bold hover:text-teal-700 dark:hover:text-teal-700 dark:text-teal-300">nicks de Free Fire pro</Link> y <Link href="/simbolos-aesthetic/simbolos-para-nick" className="text-teal-600 dark:text-teal-400 underline font-bold hover:text-teal-700 dark:hover:text-teal-700 dark:text-teal-300">símbolos para nick</Link>.
          </p>
        </header>

        {/* Interactive Hex Color Palette Grid */}
        <section className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-center">
          <div className="p-5 rounded-2xl border border-[var(--border-color)] bg-[var(--input-bg)] flex flex-col gap-1">
          ...
          <div className="p-5 rounded-2xl border border-[var(--border-color)] bg-[var(--input-bg)] flex flex-col gap-1">
            <span className="text-xs font-black text-emerald-700 dark:text-emerald-400 block font-mono">[00FF00]</span>
            <span className="text-xs font-bold text-slate-200">Verde Neón</span>
          </div>
          <div className="p-5 rounded-2xl border border-[var(--border-color)] bg-[var(--input-bg)] flex flex-col gap-1">
            <span className="text-xs font-black text-cyan-600 dark:text-cyan-400 block font-mono">[00FFFF]</span>
        ...
        <section className="flex flex-col gap-6 p-6 sm:p-10 rounded-3xl border border-[var(--border-color)] bg-[var(--card-bg)]/90 backdrop-blur-xl shadow-2xl shadow-purple-950/50">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-xs sm:text-sm">
            <div className="p-6 rounded-2xl bg-[var(--input-bg)] border border-[var(--border-color)] flex flex-col gap-2.5">
              <span className="text-xs font-extrabold text-teal-600 dark:text-teal-400 uppercase tracking-wider">🇲🇽 Bandera de México</span>
              <p className="font-mono text-slate-200 text-xs bg-[var(--background)] p-3 rounded-xl border border-[var(--border-color)]">
                [008000]█[FFFFFF]█[FF0000]█ MÉXICO
              </p>
            </div>

            <div className="p-6 rounded-2xl bg-[var(--input-bg)] border border-[var(--border-color)] flex flex-col gap-2.5">
              <span className="text-xs font-extrabold text-teal-600 dark:text-teal-400 uppercase tracking-wider">🇪🇸 Bandera de España</span>
              <p className="font-mono text-slate-200 text-xs bg-[var(--background)] p-3 rounded-xl border border-[var(--border-color)]">
